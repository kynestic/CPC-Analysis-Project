...
while True:
    driver.get(f'https://markets.businessinsider.com/news?p={page}&')
    time.sleep(3)
    ...
    for item in ele_list:
        date_str = item.find_element(By.CLASS_NAME, 'latest-news__date').get_attribute('datetime')
        
        event_str = item.find_element(By.CSS_SELECTOR, '.latest-news__title a').get_attribute('title')
